    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-800 text-white p-6 rounded-lg">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-xl font-bold mb-2">{officerProfile?.name || user?.username}</h2>
            <p className="text-blue-100 mb-1">Employee ID: {officerProfile?.employee_id || 'BMC' + user?.id}</p>
            <p className="text-blue-100 mb-1">Department: {officerProfile?.department || 'General'}</p>
            <p className="text-blue-100 mb-4">
              Assigned Wards: {assignedWardLabels.map(w => w.value).join(', ') || 'None assigned'}
            </p>
          </div>
          <div className="text-right">
            <div className="text-2xl font-bold">{performance.completion_rate || 0}%</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
          ...
          <div className="bg-white/10 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-yellow-200">{statusCounts.pending}</div>
            <div className="text-sm text-blue-100">Pending</div>
          </div>
          <div className="bg-white/10 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-green-200">{statusCounts.resolved}</div>
            <div className="text-sm text-blue-100">Resolved</div>
          </div>
          <div className="bg-white/10 rounded-lg p-3 text-center">
            <div className="text-2xl font-bold text-red-200">{statusCounts.overdue}</div>
